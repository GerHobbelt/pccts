...
static int 
compute_disjoint_union (char *set_1, char *set_2, char *set_3)
{
  char *base = set_3;
...
static void 
sort_set (char *union_set, int len)
{
  int i, j;
...
static boolean
affects_prev (char c, LIST_NODE *curr)
{
  ...
  while (--i >= 0)
    { 
      ...
      asso_values[c] = asso_values[c] + (GET_JUMP (option) ? GET_JUMP (option) : rand ())
...
static void 
change (LIST_NODE *prior, LIST_NODE *curr)
{
  ...
  if (!union_set)
    union_set = (char *)xmalloc (2 * GET_CHARSET_SIZE (option) + 1);

  if (OPTION_ENABLED (option, DEBUG)) /* Very useful for debugging. */
    {
      fprintf (report_file, "collision on keyword #%d, prior=\"%s\", curr=\"%s\", hash=%d\n",
               perfect.num_done, prior->key, curr->key, curr->hash_value);
      if (report_file != stderr)
    ...
    if (!affects_prev (*temp, curr))
      {
        if (OPTION_ENABLED (option, DEBUG))
          {
            fprintf (report_file, "- resolved by changing asso_value['%c'] (char #%d) to %d\n", 
  ...
  if (OPTION_ENABLED (option, DEBUG))
    {
      fprintf (report_file, "** collision not resolved, %d duplicates remain, continuing...\n", 
               perfect.fewest_collisions); 
      if (report_file != stderr)
  ...
  if (OPTION_ENABLED (option, DEBUG))
    {
      ...
      for (i = 0; i < ALPHABET_SIZE; i++)
        if (occurrences[i])
          fprintf (report_file, "  asso_values[%c] = %3d, occurrences[%c] = %3d\n",
                   i, asso_values[i], i, occurrences[i]);
      
      fprintf (report_file, "end table dumping\n");
