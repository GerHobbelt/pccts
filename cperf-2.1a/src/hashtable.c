...
void
hash_table_init (LIST_NODE **table, int s)
{
  hash_table.size  = s;
  hash_table.table = table;
  memset ((char *) hash_table.table, 0, hash_table.size * sizeof *hash_table.table);
}
  ...
  if (OPTION_ENABLED (option, DEBUG))
    {
      ...
      for (i = hash_table.size - 1; i >= 0; i--)
        if (hash_table.table[i])
          fprintf (report_file, "  location[%d] has charset \"%s\" and keyword \"%s\"\n",
...
#endif
      fprintf (report_file, "end dumping hash table\n\n");
    }
}
...
LIST_NODE *
retrieve (LIST_NODE *item, int ignore_length)
{
