...
static boolean determined[ALPHABET_SIZE]; 
...
static char *
get_special_input (char delimiter)
{ 
  int size  = 80;
...
save_include_src (void)
{
  ...
  {
    report_error ("internal error, %c != '{' on line %d in file %s\n"
  ...
  if (OPTION_ENABLED (option, TYPE) && !(key_list.array_type = get_array_type ()))
    ...
    {    
      ...
      if (OPTION_ENABLED (option, POINTER))      /* And it must return a pointer... */
        {    
          key_list.return_type = (char *)xmalloc (struct_tag_length + 2);
...
read_keys (void)
{
  ...
  /* Read in all the keywords from the input file. */
    ...
    {                      
      ...
      {
        /* If this becomes !0 we've got a link. */
        int link = 0;  
        ...
        LIST_NODE **table = (LIST_NODE **) xmalloc (POW (table_size) * sizeof (LIST_NODE *));
            ...
            if (ptr)              
              {                   
                ...
                if (!OPTION_ENABLED (option, DUP))
                  fprintf (report_file, "Key link: \"%s\" = \"%s\", with key set \"%s\".\n", 
      ...
      if (key_list.min_key_len == 0)
        {
          report_error("Empty input key is not allowed.\nTo recognize an empty input key, your code should check for\nlen == 0 before calling the gperf generated lookup function.\n");
        }
...
static LIST_NODE *
merge (LIST_NODE *list1, LIST_NODE *list2)
{
  if (!list1)
    return list2;
  else if (!list2)
...
static boolean 
already_determined (LIST_NODE *ptr)
{
  boolean  is_determined = TRUE;
  char *temp;
...
print_switch (void)
{
  ...
  if (pointer_and_type_enabled)
    ...
    strcpy(comp_buffer, (OPTION_ENABLED (option, COMP) 
...
print_keylength_table (void)
{
  ...
  if (
#if 0 /* [i_a] fix: -D -S should provide a 'lengthtable' */
      !OPTION_ENABLED (option, DUP) && !OPTION_ENABLED (option, SWITCH)
#else
	...
	for (temp = key_list.head; temp; temp = temp->next, index++)
		{
					...
					for (column = 1; index < temp->hash_value; index++, column++)
          {
			...
			printf ("%s  %s\"%s\", %s%s\n", indent, l_brace, temp->key, temp->rest, r_brace);
			...
			if (temp->link)
				{
					...
					for (links = temp->link; links; links = links->link)
						{
							links->index = ++index;
							printf ("%s  %s\"%s\", %s%s\n", indent, l_brace, links->key, links->rest, r_brace);
						}
				}
...
static void 
print_hash_function (int max_hash_value)
{
  int max_column = 10;
  ...
  printf (OPTION_ENABLED (option, ANSI) 
          ...
          ? "char" : (max_hash_value < MAX_UNSIGNED_SHORT ? "short int" : "int"));
  ...
  for (count = 0; count < ALPHABET_SIZE; ++count)
    {
      if (!(count % max_column))
      {
        if (OPTION_ENABLED (option, DEBUG)
            && count > 0)
        {
          int start_count = count - max_column;

          if (start_count % max_column)
            start_count += max_column - (start_count % max_column);

          printf(" /* 0x%02X [%c] .. 0x%02X [%c] */"
                ,start_count
                ,(isprint(start_count) ? (char)start_count : ' ')
                ,count - 1
                ,(isprint(count - 1) ? (char)(count - 1) : ' ')
                );
        }
        printf ("\n    ");
      }
  ...
  {
    int start_count = count - max_column;
    int end_count = count + max_column - 1;

    if (start_count % max_column)
      start_count += max_column - (start_count % max_column);
    if (end_count % max_column)
      end_count -= (end_count % max_column);

    printf("%*s /* 0x%02X [%c] .. 0x%02X [%c] */"
          ,(end_count - count) * (field_width + 1)
          ,""
          ,start_count
          ,(isprint(start_count) ? (char)start_count : ' ')
          ,count - 1
...
{      
  ...
  for (ptr = key_list.head; ptr; ptr = ptr->next)
    fprintf (report_file, "   %10d, %10d, %6d, %7s, %s\n",
  ...
  if (OPTION_ENABLED (option, DEBUG))
    {
      ...
      dump ();
      fprintf (report_file, "End dumping list.\n"
                       "\n");
    }
}
