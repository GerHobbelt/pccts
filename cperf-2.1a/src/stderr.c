...
void 
set_program_name (char *prog_name)
{ 
  program_name = prog_name;
}

/* return pointer to location where LAST occurrence of any char in SET was found. */
char *strrset(char *src, char *set)
{
  char *res = NULL;
  char *p = src;
      ...
      if (*format != '%') 
      ...
      else 
        {
          switch(*++format) 
            ...
            case 'n' : fputs (program_name 
                              ? ( strrset(program_name, "/\\:]") != NULL
                                ? strrset(program_name, "/\\:]") + 1
                                : program_name )
                              : "???", stderr); break;
            case 'p' : 
              if (errno < sys_nerr) 
