...
bool_array_destroy (void)
{
  ...
  xfree ((void **) &bool_array.storage_array);
}

void
bool_array_init (int size)
{
  bool_array.iteration_number = 1;
  bool_array.size = size;
  bool_array.storage_array = (STORAGE_TYPE *)xmalloc (size * sizeof *bool_array.storage_array);
  ...
  if (++bool_array.iteration_number == 0)
    {
      if (OPTION_ENABLED (option, DEBUG))
        {
          fprintf (report_file, "(re-initializing bool_array)...");
          fflush (report_file);
        }
      bool_array.iteration_number = 1;
