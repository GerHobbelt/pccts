...
rem
rem this batch file is for MSVC 6.0; for MSVC 7.0 (.NET) you'll need a third party profiler, e.g. the one
rem in the NuMega DevPartner toolset, which also contains the superb BoundsChecker error detection and
rem analysis tool, which is indispensible for any serious C/C++ developer. (Nope, I don't have
rem Compuware/NuMega shares. It's a simple fact of (developer) life.)
rem
