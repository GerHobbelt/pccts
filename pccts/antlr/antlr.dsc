...
element :
	  { element_label } ( TokenTerm { ".." ( QuotedTerm | TokenTerm ) } ( "^" | | "!" ) { "\@" } | QuotedTerm { ".." ( QuotedTerm | TokenTerm ) } ( "^" | | "!" ) { "\@" } | WildCard ( "^" | | "!" ) | NonTerminal { "!" } { { "\<" } PassAction } { "\>" PassAction } )
	| Action
	| Pred { PassAction }
	| { Pragma ( "approx" | "LL\(1\)" | "LL\(2\)" ) } { FirstSetSymbol "\(" ( NonTerminal | TokenTerm ) "\)" } ( "\(" block "\)" ( "\*" | "\+" | "?" ( ( "=>" | "&&" ) Pred { PassAction } | ) | ) | "\{" block "\}" )
	| "\*"
	| "\+"
	| "\>"
	| PassAction
...
enum_file :
	  { "#ifndef" ID { "#define" ID { INT } } } ( ( enum_def )+ | defines )
	|
