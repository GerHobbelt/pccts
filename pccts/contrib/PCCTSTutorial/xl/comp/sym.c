...
int sz, strs;
{
	...
	if ( table == NULL )
	{
		printf("Cannot allocate table of size %d\n", sz);
		exit(1);
	}
	strings = (char *) calloc(strs, sizeof(char));
	if ( strings == NULL )
	{
		printf("Cannot allocate string table of size %d\n", strs);
		exit(1);
	}
	size = sz;
	strsize = strs;
...
register char *s;
{
	...
	while ( *s != '\0' )
	{
		...
		{
			printf("sym: string table overflow (%d chars)\n", strsize);
			exit(-1);
		}
		*strp++ = *s++;
	}
