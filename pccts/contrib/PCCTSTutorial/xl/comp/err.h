...
#endif
{
	...
	if ( zzset_deg(a)>1 ) printf(" {");
	do {
		register SetWordType t = *p;
		register SetWordType *b = &(bitmask[0]);
		do {
			if ( t & *b ) printf(" %s", zztokens[e]);
			e++;
...
{
	error_occurred = 1;
	printf("line %d: syntax error at \"%s\"", zzline, (tok==zzEOF_TOKEN)?"EOF":bad_text);
	if ( !etok && !eset ) {printf("\n"); return;}
	if ( k==1 ) printf(" missing");
	else
	{
		printf("; \"%s\" not", bad_text);
		if ( zzset_deg(eset)>1 ) printf(" in");
	}
	if ( zzset_deg(eset)>0 ) zzedecode(eset);
	else printf(" %s", zztokens[etok]);
	if ( strlen(egroup) > 0 ) printf(" in %s", egroup);
	printf("\n");
}
#endif
...
#endif
{
	...
	{
		printf("cannot allocate lookahead text buffer (%d bytes)\n", 
	...
	{
		printf("cannot allocate token buffer (%d tokens)\n", 
    ...
    if ( zzinf_line == NULL )
    {
        printf("cannot allocate line buffer (%d ints)\n",
                zzinf_token_buffer_size);
        exit(-1);
	}
	...
	do {
		...
		while ( zzinf_lap>=zzinf_token_buffer_size )
		{
			...
			if ( zzinf_tokens == NULL )
			{
				printf("cannot allocate lookahead token buffer (%d tokens)\n", 
						zzinf_token_buffer_size);
				exit(-1);
			}
            ...
            if ( zzinf_line == NULL )
            {
                printf("cannot allocate lookahead line buffer (%d ints)\n",
                        zzinf_token_buffer_size);
                exit(-1);
			}
	...
	if ( zzinf_text == NULL )
	{
		printf("cannot allocate lookahead text buffer (%d)\n", 
				zzinf_text_buffer_size); 
		exit(-1);										    
	}													    
	zzinf_text_buffer_index = 0;
